...
#	{
#		"version": "2.3.0",
#		"content": "<content of release note (markdown format) without carriage return(replace with <cr>)>"
#	},
#	{
#		"version": "2.2.0",
#		"content": "<content of release note (markdown format) without carriage return(replace with <cr>)>"
...
convert_md_to_json() {
    ...
    do
        ...
        # Reading each line and convert CR with "<CR>"
        while read line; do
            echo -n "$line<cr>" >> "$output_file"
        done < "${file}.md"
        echo -n $'"\n\t}' >> "$output_file"
        n=$((n+1))
    done
